...
import { Loader2, MessageCircle, UserPlus, UserMinus, Settings, Pencil, CalendarDays, Users, FileText, ShieldCheck } from "lucide-react";
...
function UserProfilePage() {
  ...
  async function startMessage() {
    ...
    if (shared) {
      navigate({ to: "/messages/$conversationId", params: { conversationId: shared.conversation_id } });
      return;
    }
  ...
  if (isLoading) {
    return (
      <div className="flex items-center justify-center min-h-[60vh]">
        <Loader2 className="h-8 w-8 animate-spin text-[oklch(0.78_0.18_220)]" />
      </div>
    );
  }
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] gap-4">
        ...
        <Button asChild variant="outline">
          <Link to="/feed">العودة للرئيسية</Link>
        </Button>
      </div>
    );
  }
      ...
      <div className="relative -mt-20 px-4 sm:px-6">
        <div className="flex items-end gap-4">
          <div className="relative">
            ...
            {profile.is_online && (
              <span className="absolute bottom-1 right-1 h-4 w-4 rounded-full bg-green-500 border-2 border-[oklch(0.03_0.01_260)] shadow-[0_0_8px_oklch(0.5_0.2_145/0.5)]" />
            )}
          </div>
              ...
              <>
                <Button
                  ...
                  className={isFollowing
                    ? "bg-[oklch(0.06_0.015_260/0.5)] border border-[oklch(1_0_0/0.1)] text-[oklch(0.8_0.03_250)] hover:bg-destructive/10 hover:text-destructive"
                ...
                >
                  {followMut.isPending ? (
                    ...
                    <><UserMinus className="h-3.5 w-3.5 mr-1" /> إلغاء المتابعة</>
                  ) : (
                    <><UserPlus className="h-3.5 w-3.5 mr-1" /> متابعة</>
                  )}
                </Button>
                <Button
                  size="sm"
                  ...
                  className="border-[oklch(1_0_0/0.1)] bg-[oklch(0.06_0.015_260/0.5)]"
                >
        ...
        <div className="mt-3">
          <div className="flex items-center gap-2">
            ...
            {profile.is_verified && (
              <ShieldCheck className="h-5 w-5 text-[oklch(0.78_0.18_220)]" />
            )}
          </div>
      ...
      <div className="mt-6 space-y-4 px-2">
        ...
        {postsLoading && (
          <div className="text-center py-8">
            <Loader2 className="h-6 w-6 animate-spin mx-auto text-[oklch(0.78_0.18_220)]" />
          </div>
        )}
        {!postsLoading && posts.length === 0 && (
          <div className="text-center py-12 text-[oklch(0.45_0.03_250)] text-sm">
            لا توجد منشورات بعد
          </div>
        )}
        {posts.map((p) => (
          <PostCard key={p.id} post={p} onChange={() => qc.invalidateQueries({ queryKey: ["user-posts", userId] })} />
        ))}
      </div>
