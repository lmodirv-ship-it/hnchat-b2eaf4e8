    ...
    <div className="min-h-screen bg-background text-foreground overflow-x-hidden" dir={isRTL ? "rtl" : "ltr"}>
      ...
      <section className="relative z-10 max-w-7xl mx-auto px-3 sm:px-6 pt-4 sm:pt-8 pb-8 sm:pb-12 flex flex-col lg:flex-row gap-8 sm:gap-10 items-center">
        ...
        <div className="flex-1 text-center lg:text-start pt-2 sm:pt-4 max-w-2xl">
          ...
          <motion.div className="flex justify-center lg:justify-start mb-6" initial={init} animate={enter} variants={fadeUp} custom={0}>
            <button
              onClick={async () => {
                try {
              ...
              className="group relative flex items-center gap-3 px-8 sm:px-10 py-4 sm:py-5 rounded-2xl border-2 border-ice-border/50 bg-white/[0.07] backdrop-blur-2xl text-foreground font-bold text-base sm:text-lg transition-all duration-300 hover:border-white/40 hover:bg-white/[0.12] hover:shadow-[0_0_40px_oklch(0_0_1/0.15)] hover:scale-[1.03] active:scale-[0.97] cursor-pointer"
            >
              <svg className="h-6 w-6 sm:h-7 sm:w-7 shrink-0" viewBox="0 0 24 24">
                <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 0 1-2.2 3.32v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.1z" fill="#4285F4"/>
                <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853"/>
                <path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="#FBBC05"/>
                <path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335"/>
              </svg>
              <span className="bg-gradient-to-r from-white via-white/90 to-white/80 bg-clip-text text-transparent drop-shadow-sm">
                {lang === "ar" ? "الدخول بحساب Google" : lang === "fr" ? "Continuer avec Google" : lang === "es" ? "Continuar con Google" : lang === "de" ? "Weiter mit Google" : lang === "tr" ? "Google ile devam et" : lang === "pt" ? "Continuar com Google" : lang === "zh" ? "使用 Google 登录" : lang === "ru" ? "Войти через Google" : "Continue with Google"}
              </span>
            </button>
          </motion.div>
          ...
          <motion.div className="flex justify-center lg:justify-start mt-4" initial={init} animate={enter} variants={fadeUp} custom={3.5}>
            <button
              onClick={async () => {
                try {
                  const result = await lovable.auth.signInWithOAuth("google", {
