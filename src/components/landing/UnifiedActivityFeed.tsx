...
import { Newspaper, Youtube, MessageSquare, Radio, Play, Activity, RefreshCw } from "lucide-react";
...
const kindMeta = {
  article: { label: { ar: "مقال", en: "Article" }, icon: Newspaper, color: "from-blue-500 to-cyan-500", badge: "bg-blue-500/15 text-blue-300 border-blue-500/30" },
  video: { label: { ar: "فيديو", en: "Video" }, icon: Youtube, color: "from-red-500 to-pink-500", badge: "bg-red-500/15 text-red-300 border-red-500/30" },
  post: { label: { ar: "منشور", en: "Post" }, icon: MessageSquare, color: "from-violet-500 to-fuchsia-500", badge: "bg-violet-500/15 text-violet-300 border-violet-500/30" },
  live: { label: { ar: "بث مباشر", en: "Live" }, icon: Radio, color: "from-rose-500 to-orange-500", badge: "bg-rose-500/15 text-rose-300 border-rose-500/30" },
} as const;
...
export function UnifiedActivityFeed({ lang = "ar" }: { lang?: string }) {
  ...
  const load = async () => {
    ...
    const [articlesRes, videosRes, postsRes, livesRes] = await Promise.all([
      ...
      supabase
        .from("channel_videos")
        .select("id, video_id, title, thumbnail, author, published_at_app, published_at")
      ...
      supabase
        .from("posts")
        .select("id, content, media_urls, type, created_at, user_id, profiles:profiles!posts_user_id_fkey(full_name, username, avatar_url)")
              ...
              return (
                ...
                >
                  ...
                  <div className="p-4 flex flex-col gap-2 flex-1">
                    ...
                    {it.author && <p className="text-[11px] text-white/40 mt-auto truncate">— {it.author}</p>}
                  </div>
                </Link>
              );
            })}
