                ...
                return (
                  <div key={gi} className={cn("flex gap-2", mine ? "justify-end" : "justify-start")}>
                    ...
                    <div className={cn("flex flex-col gap-0.5 max-w-[75%]", mine ? "items-end" : "items-start")}>
                      {!mine && isGroup && (
                        <Link to="/profile/$username" params={{ username: members[group.senderId]?.username ?? group.senderId }} className="text-[11px] font-medium text-cyan-glow/90 px-1 hover:text-cyan-glow transition-colors">
